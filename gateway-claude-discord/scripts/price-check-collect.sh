...
# ── 8. 포트폴리오 현황 ────────────────────────────────────────────────────────
echo "### 전략 조건 참조표 (30분 크론 사용 전략: id=1,5,6만 출력)"
echo "$STRATEGIES" | jq -r '.strategies[] | select(.id == 1 or .id == 5 or .id == 6) | "  [id=\(.id)] \(.name)\n\(.notes // "(조건 없음)")\n---"'
echo ""
...
while IFS='|' read -r PF_ID PF_NAME NOTIFY RISK_LIMIT; do
  ...
  for COIN in $COINS; do
    ...
    POSITION=$(echo "$STATUS" | jq -r '.position // "UNKNOWN"')
    CASH=$(echo "$STATUS" | jq -r '(.cash // 0) | round')
    VALUE=$(echo "$STATUS" | jq -r '(.current_value // 0) | round')
    ROI=$(echo "$STATUS" | jq -r '(.return_pct // 0) | . * 10 | round | . / 10')
    AVG_COST=$(echo "$STATUS" | jq -r '.avg_cost // 0 | round')
    MARKET_PRICE=$(echo "$STATUS" | jq -r '(.current_price // 0) | round')

    STRAT_NAME=$(echo "$HIST" | jq -r '.history[0].strategy_name // "없음"')
    STRAT_ID=$(echo "$HIST" | jq -r '.history[0].strategy_id // 0')

    echo "  ${COIN}: ${POSITION} | 시장단가:${CURRENCY}${MARKET_PRICE} | 포트폴리오총자산:${CURRENCY}${VALUE} (ROI:${ROI}%) | cash:${CURRENCY}${CASH} | avg_cost:${CURRENCY}${AVG_COST} | 전략:${STRAT_NAME}(id=${STRAT_ID})"
  done
done <<< "$PF_META"
...
echo "※ pf1-2,4,17: 바이낸스 지표 + USD 가격"
echo "※ pf5-6,8,10,19: 업비트 지표 + KRW 가격"
echo "※ pf12-13,15-16,21: 빗썸 지표 + KRW 가격"
echo "※ pf10(김치프리미엄): 외부시그널 김프 값 기준 + 업비트 KRW 가격"
echo "※ pf16(빗썸 김치프리미엄): 외부시그널 빗썸김프 값 기준 + 빗썸 KRW 가격"
echo "※ pf3/7/14(수동매매), pf11(뉴스센티멘트): 이 크론잡 제외 — 수동 또는 별도 job 처리"
echo "※ pf9/23/24(Fear&Greed): 별도 fear-greed-trade 크론잡이 처리 (09:05, 21:05 KST)"
echo "※ pf18/20/22(BTC 도미넌스): 별도 btc-dominance-trade 크론잡이 처리 (2시간마다)"
