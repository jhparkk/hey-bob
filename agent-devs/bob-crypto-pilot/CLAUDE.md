...
│   ├── upbit.go             ← 업비트 ticker/시뮬레이션 핸들러
│   └── bithumb.go           ← 빗썸 ticker/시뮬레이션 핸들러
├── models/
...
│   ├── ticker.go            ← 10초 간격 일봉 지표 갱신 (Binance)
│   ├── hourly_ticker.go     ← 10분 간격 1시간봉 지표 갱신 (Binance)
│   ├── upbit.go             ← 업비트 API 호출
│   ├── upbit_ticker.go      ← 업비트 일봉 지표 갱신
│   ├── upbit_hourly_ticker.go ← 업비트 1시간봉 지표 갱신
│   ├── bithumb.go           ← 빗썸 API 호출
│   ├── bithumb_ticker.go    ← 빗썸 일봉 지표 갱신
│   ├── bithumb_hourly_ticker.go ← 빗썸 1시간봉 지표 갱신
...
└── fe/                      ← React+TypeScript 프론트엔드
    ...
    │   ├── pages/           ← ChartPage, SimulationPage, StrategyPage, UpbitSimulationPage, BithumbSimulationPage
    │   ├── hooks/           ← useSimulation, useStrategy, useUpbitSimulation, useBithumbSimulation
...
GET    /api/v1/upbit/simulation/portfolios      업비트 포트폴리오 요약
GET    /api/v1/upbit/simulation/performance     업비트 기간별 수익률
POST   /api/v1/upbit/sync                       업비트 데이터 동기화

# Bithumb
GET    /api/v1/bithumb/ticker                   빗썸 일봉 지표
GET    /api/v1/bithumb/ticker/hourly            빗썸 1시간봉 지표
GET    /api/v1/bithumb/simulation/portfolios    빗썸 포트폴리오 요약
GET    /api/v1/bithumb/simulation/performance   빗썸 기간별 수익률
POST   /api/v1/bithumb/sync                     빗썸 데이터 동기화
...
| id | 이름 | 설명 | 적합 환경 |
|----|------|------|----------|
| 1 | 분할매수형 | 보수적 분할매수 | 횡보/하락 |
| 2 | 모멘텀형 | 공격적 모멘텀 추종 | 상승 추세 |
| 4 | 공격형 | 고위험 단기 전략 | 과매도 반등 |
| 5 | Box Hunter | BB하단+RSI과매도 매수, 분할 익절 | 박스권 |
| 6 | Trend Rider | 1시간봉 EMA 크로스 + MACD 모멘텀 (trend-rider-optimizer가 매일 08:30 자동 업데이트) | 모든 국면 |
| 7 | Fear & Greed 역발상 | 공포탐욕지수 기반 역발상 매매 | 공포 구간 |
| 8 | 김치프리미엄 | 업비트-바이낸스 가격 괴리 역발상 | 김프 상승 시 |
| 9 | 뉴스센티멘트 | 뉴스 감성분석 기반 이벤트 드리븐 | 이벤트 발생 시 |
...
### Binance (바이낸스)
| id | 이름 | 활성 전략 | notify | risk |
|----|------|----------|--------|------|
| 1 | [바낸] 분할 매수 | 분할매수형(id=1) | on | 15% |
| 2 | [바낸] Box Hunter | Box Hunter(id=5) | on | 15% |
| 3 | [바낸] 수동 매매 | (수동) | off | 15% |
| 4 | [바낸] Trend Rider | Trend Rider(id=6) | on | 10% |
| 9 | [바낸] Fear&Greed | Fear&Greed(id=7) | on | 15% |

### Upbit (업비트)
| id | 이름 | 활성 전략 | notify | risk |
|----|------|----------|--------|------|
| 5 | [업비트] 분할 매수 | 분할매수형(id=1) | on | 15% |
| 6 | [업비트] Box Hunter | Box Hunter(id=5) | on | 15% |
| 7 | [업비트] 수동 매매 | (수동) | on | 15% |
| 8 | [업비트] Trend Rider | Trend Rider(id=6) | on | 15% |
| 10 | [업비트] 김치프리미엄 | 김치프리미엄(id=8) | on | 15% |

### Bithumb (빗썸)
| id | 이름 | 활성 전략 | notify | risk |
|----|------|----------|--------|------|
| 12 | [빗썸] 분할 매수 | 분할매수형(id=1) | on | 15% |
| 13 | [빗썸] Box Hunter | Box Hunter(id=5) | on | 15% |
| 14 | [빗썸] 수동 매매 | (수동) | on | 15% |
| 15 | [빗썸] Trend Rider | Trend Rider(id=6) | on | 15% |
| 16 | [빗썸] 김치프리미엄 | 김치프리미엄(id=8) | on | 15% |

| id=11 | [바낸] 뉴스센티멘트 | 뉴스센티멘트(id=9) | on | 15% |
...
| 📈 바낸 시뮬 | SimulationPage.tsx | 바이낸스 포트폴리오 요약, 기간별 수익률 비교, 자산 카드, 거래 히스토리, ROI 차트 |
| 🇰🇷 업비트 시뮬 | UpbitSimulationPage.tsx | 업비트 포트폴리오 요약 및 성과 |
| 🟡 빗썸 시뮬 | BithumbSimulationPage.tsx | 빗썸 포트폴리오 요약 및 성과 |
...
- DB 경로: `os.Executable()` 기준 상대경로 → 바이너리 위치 기준으로 항상 고정됨
