...
import { useBithumbSimulation } from '../hooks/useBithumbSimulation';
import { executeTrade, updatePortfolio, getBithumbSimPerformance, getBithumbTicker } from '../api';
...
const BithumbSimulationPage: React.FC<{ isActive?: boolean }> = ({ isActive }) => {
  ...
  useEffect(() => {
    getBithumbSimPerformance().then(r => setPerformance(r.portfolios)).catch(() => {});
  }, []);
  ...
  const handleTradeConfirm = async (amount: number) => {
    ...
    try {
      const tickerData = await getBithumbTicker();
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ border: '1px solid #2a2a4a', borderTop: 'none', borderRadius: '0 0 8px 8px', padding: 16, background: '#12121f' }}>
        ...
        <PortfolioROIChart portfolioId={currentPortfolioId} exchange="bithumb" />
        ...
        <div style={{ border: '1px solid #2a2a4a', borderTop: 'none', borderRadius: '0 0 6px 6px', padding: 14, background: '#0f0f1f', minHeight: 320 }}>
          ...
          {!loading && !error && simStatus && (
            <>
              <BithumbAssetCard status={simStatus} coin={currentCoin} />
...
const BithumbAssetCard: React.FC<{ status: SimStatus; coin: string }> = ({ status, coin }) => {
...
export default BithumbSimulationPage;
