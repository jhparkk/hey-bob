...
import { getLivePrice, executeTrade, updatePortfolio, getSimPerformance } from '../api';
...
const SimulationPage: React.FC = () => {
  ...
  useEffect(() => {
    getSimPerformance().then(r => setPerformance(r.portfolios)).catch(() => {});
  }, []);
  ...
  const loadAll = useCallback(async () => {
    ...
    if (portfolios.length > 0) {
      // store에서 최신값 직접 읽기 → stale closure 없음
      const currentId = useSimulationStore.getState().currentPortfolioId;
      const currentC = useSimulationStore.getState().currentCoin;
      const pfId = portfolios.find(i => i.portfolio?.id === currentId)
        ? currentId
        : portfolios[0].portfolio?.id ?? 1;
      setCurrentPortfolioId(pfId);
      const pf = portfolios.find(i => i.portfolio?.id === pfId);
      const pfCoins = pf?.states?.map(s => s.coin) || [];
      const activeCoin = pfCoins.includes(currentC) ? currentC : (pfCoins[0] || 'BTC');
      setCurrentCoin(activeCoin);
      await loadSimCoinDetail(activeCoin, pfId);
    }
    await loadTickers();
  // deps 비어도 OK: store.getState()로 항상 최신값 참조
  ...
  const handleTradeConfirm = async (amount: number) => {
    ...
    try {
      const liveData = await getLivePrice(currentCoin);
  ...
  return (
    <div style={{ padding: 16 }}>
            ...
            {simPortfolios.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: 8, textAlign: 'right' }}>${totalAsset.toFixed(2)}</td>
      ...
      <div style={{ border: '1px solid #2a2a4a', borderTop: 'none', borderRadius: '0 0 8px 8px', padding: 16, background: '#12121f' }}>
        ...
        {/* ROI Chart */}
        <PortfolioROIChart portfolioId={currentPortfolioId} />
        ...
        <div style={{ border: '1px solid #2a2a4a', borderTop: 'none', borderRadius: '0 0 6px 6px', padding: 14, background: '#0f0f1f', minHeight: 320 }}>
          ...
          {!loading && !error && simStatus && (
            <>
              {/* Asset card */}
              <SimAssetCard status={simStatus} coin={currentCoin} />
      ...
      <EditPortfolioModal
        ...
        onSaved={async () => {
          // store에서 최신값 직접 접근 → stale 없음
          const { currentPortfolioId: savedPfId, currentCoin: savedCoin } = useSimulationStore.getState();
          setEditPf(null);
          const portfolios = await loadSimPortfolios();
          setCurrentPortfolioId(savedPfId);
          // 삭제된 코인이면 첫 번째 코인으로 fallback
          const pf = portfolios.find((i: { portfolio?: { id: number } }) => i.portfolio?.id === savedPfId);
          const pfCoins: string[] = pf?.states?.map((s: { coin: string }) => s.coin) || [];
          const activeCoin = pfCoins.includes(savedCoin) ? savedCoin : (pfCoins[0] || 'BTC');
          setCurrentCoin(activeCoin);
          await loadSimCoinDetail(activeCoin, savedPfId);
          await loadTickers();
        }}
      />
    </div>
...
const SimAssetCard: React.FC<{ status: SimStatus; coin: string }> = ({ status, coin }) => {
